...
function startScheduler() {
  ...
  schedulerTimer = setInterval(() => {
    ...
    while (noteIndex < parsedNotes.length && parsedNotes[noteIndex].start <= scheduleUntil) {
      ...
      const noteTime = songStartTime + note.start;
    ...
    if (elapsed >= songDuration + 0.5) {
      stopCurrentSong();
      if (typeof currentOnEnd === 'function') currentOnEnd();
    }
  }, 40);
}

// ---------------------------------------------------------------------------
// 7. Song laden und abspielen
// ---------------------------------------------------------------------------
async function playSong(midiFilePath, onEnd) {
  ...
  try {
    ...
    isPausedState   = false;
    pausedAt        = 0;
    songStartTime   = audioCtx.currentTime + 0.1;
    startTimeSec    = songStartTime;
...
// ---------------------------------------------------------------------------
// 8. Song pausieren / fortsetzen / neu starten
// ---------------------------------------------------------------------------
function pauseSong() {
  ...
  if (audioCtx) {
    audioCtx.suspend();
    pausedAt = audioCtx.currentTime - songStartTime;
  }
  isPausedState = true;
}

function resumeSong() {
  if (!isPausedState || !audioCtx) return Promise.resolve(false);
...
function restartSong(path, onEnd) {
  const targetPath = path || currentMidiPath;
  const targetOnEnd = onEnd || currentOnEnd;
  stopCurrentSong();
  if (targetPath) {
    return playSong(targetPath, targetOnEnd);
  }
}

function getProgress() {
  if (!audioCtx || songDuration === 0) {
    return { elapsed: 0, duration: 0 };
  }
  const elapsed = isPausedState ? pausedAt : (audioCtx.currentTime - songStartTime);
  return {
...
function seekTo(targetSec) {
  ...
  if ((!parsedNotes || parsedNotes.length === 0) && currentMidiPath && midiCache[currentMidiPath]) {
    parsedNotes = midiCache[currentMidiPath].notes;
    songDuration = midiCache[currentMidiPath].duration;
  }
  if (!parsedNotes || songDuration === 0) return;

  targetSec = Math.max(0, Math.min(targetSec, songDuration));

  // Stop currently active notes
  activeOscillators.forEach(o => { try { o.stop(); } catch (e) {} });
  activeOscillators = [];

  // Locate the note index
  noteIndex = 0;
  while (noteIndex < parsedNotes.length && parsedNotes[noteIndex].start < targetSec) {
    noteIndex++;
  }

  if (isPausedState || !isPlaying) {
    pausedAt = targetSec;
    isPausedState = true;
    isPlaying = false;
    songStartTime = audioCtx.currentTime - targetSec;
    startTimeSec = songStartTime;
  } else {
    songStartTime = audioCtx.currentTime - targetSec;
    startTimeSec = songStartTime;
